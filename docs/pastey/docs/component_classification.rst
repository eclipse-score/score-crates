...
.. list-table:: Determine (P)
        ...
        * - 2
          ...
          - PE
          - Functional requirements are formally captured in this repository and organized into three levels:

            * Assumed system requirements: `assumed_system_requirements.trlc <requirement/assumed_system_requirements.trlc>`_
            * Feature requirements: `feature_requirements.trlc <requirement/feature_requirements.trlc>`_
            * Component requirements: `component_requirements.trlc <requirement/component_requirements.trlc>`_

            The crate documentation at https://docs.rs/pastey/0.2.3/pastey/ provides supplementary usage descriptions and examples.

        * - 3
          - Are specifications for functionalities and properties available (architecture)?
          - PE
          - Architecture and interface specifications are formally documented in this repository:

            * Component architecture document: :doc:`architecture/index`
            * Static design diagram: `static_design.puml <architecture/static_design.puml>`_ 
            * Public API interface diagram: `public_api.puml <architecture/public_api.puml>`_

            The crate documentation at https://docs.rs/pastey/0.2.3/pastey/ provides supplementary feature descriptions, usage examples, and modifier tables.

        * - 4
          - Are design specifications available?
          - PE
          - Design specifications are formally captured in this repository using UML diagrams:

            * Static architecture: `static_design.puml <architecture/static_design.puml>`_
            * Public interface: `public_api.puml <architecture/public_api.puml>`_ 

            Both diagrams are accompanied by the architecture narrative in :doc:`architecture/index`.
        ...
        * - 6
          ...
          - HE
          - Comprehensive test suite exists with full traceability to component requirements. Integration tests in `pastey_test.rs <../tests/pastey_test.rs>`_`

            * Upstream test suite: https://github.com/AS1100K/pastey/tree/master/pastey-test-suite
            * CI test results: https://github.com/AS1100K/pastey/pull/30/checks (or run locally with ``cargo test``)

            An LLVM-based code coverage report is generated and available at the CI link above, with:

            * 100% function coverage
            * 100% line coverage
            * 100% branch coverage

            (coverage report: https://github.com/AS1100K/pastey/pull/30/checks)
...
Traceability:
  End-to-end traceability has been established using the LOBSTER tool within the score-crates repository. All necessary traceability links are properly maintained, covering mappings between:

  * Assumed system requirements: `assumed_system_requirements.trlc <requirement/assumed_system_requirements.trlc>`_
  * Feature requirements: `feature_requirements.trlc <requirement/feature_requirements.trlc>`_
  * Component requirements: `component_requirements.trlc <requirement/component_requirements.trlc>`_
  * Architecture and design: :doc:`architecture/index`, `static_design.puml <architecture/static_design.puml>`_, `public_api.puml <architecture/public_api.puml>`_
  * Safety analysis — failure modes: `failure_modes.trlc <safety_analysis/failure_modes.trlc>`_ (``UncertifiedRustCompiler``)
  * Safety analysis — assumptions of use: `aou.trlc <safety_analysis/aou.trlc>`_ (``PasteyRustCompilerCertified``)
  * Safety analysis — fault tree: `uncertified_rust_compiler_fta.puml <safety_analysis/root_causes/uncertified_rust_compiler_fta.puml>`_
  * Tests: `pastey_test.rs <../tests/pastey_test.rs>`_

  Although unit test results are not currently displayed in the generated LOBSTER reports due to a known limitation in Rust test result parsing (https://github.com/bmw-software-engineering/lobster/issues/589), all traceability links are present and valid.

Toolchain Assurance:
  The crate is built and tested using the certified Ferrocene toolchain, which is maintained as part of the score_rust_toolchain repository. This ensures compliance with the ``PasteyRustCompilerCertified`` assumption of use defined in `aou.trlc <safety_analysis/aou.trlc>`_ and mitigates the ``UncertifiedRustCompiler`` failure mode documented in `failure_modes.trlc <safety_analysis/failure_modes.trlc>`_.
